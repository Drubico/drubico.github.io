...
async function populateTimeline(timelineElement, items, section) {
    ...
    items.forEach((item, index) => {
        ...
        // Contenedor para la columna izquierda (logo)
        const leftCol = document.createElement("div");
        leftCol.className = "timeline-col timeline-col-left";

        // Usar logoId estándar
        if (item.logoId && experienceLogos[item.logoId]) {
            const logo = document.createElement("img");
            logo.src = experienceLogos[item.logoId];
            logo.alt = item.logoId + ' logo';
